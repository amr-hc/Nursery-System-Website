...
exports.insert=[
    body("supervisor").isInt().withMessage("id Must be an integer").custom(async (supervisor_id)=>{
    ...
    body("children").isArray().custom(async (children_id)=>{
...
exports.update=[
    ...
    body("supervisor").optional().isInt().withMessage("supervisor must be an integer").custom(async (supervisor_id)=>{
                const supervisor = await TeacherSchema.findOne({_id: supervisor_id});
                if( supervisor == null ){
                    return Promise.reject("This supervisor is not Exist");
                }else{
    ...
    body("children").optional().isArray().custom(async (children_id)=>{
                const children = await childSchema.find({_id: { $in : children_id }})
                if(children.length != children_id.length){
                    return Promise.reject("there is id children not Exist");
                }else{
